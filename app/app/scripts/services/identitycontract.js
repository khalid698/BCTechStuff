    ...
    */
    self.attest = function(identity, target, assertionType){
      ...
      if(!target.contractAddress){
        p.reject("Cannot attest target without contract address");
      } else {
          self.createIdentityClient(identity, target.contractAddress, self.web3Options, self.web3PromiseResolver(p)).attest(assertionType.id);
      }
      return p;
    };
